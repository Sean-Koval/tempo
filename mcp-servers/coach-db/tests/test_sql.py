"""Tests for coach_db_mcp.sql — fixture DB, pure-function semantics."""
...
def test_query_activities_filters_all_params(conn: sqlite3.Connection) -> None:
    ...
    # sport filter
    got = sql.query_activities(conn, sport="ride")
    assert [a.id for a in got] == ["a2", "a1"]  # newest first

    # decoupling cap filters out a2 (8.1%) but keeps a1 (3.5%)
    got = sql.query_activities(conn, sport="ride", min_duration_s=10000,
                               max_decoupling_pct=5.0)
    assert [a.id for a in got] == ["a1"]

    # date window
    got = sql.query_activities(conn, start="2026-04-02", end="2026-04-03")
...
def test_query_activities_limit(conn: sqlite3.Connection) -> None:
    ...
    got = sql.query_activities(conn, limit=2)
    assert len(got) == 2
...
def test_get_load_curve_returns_inclusive_range(conn: sqlite3.Connection) -> None:
    ...
    pts = sql.get_load_curve(conn, start_date="2026-03-31", end_date="2026-04-01")
...
def test_get_readiness_computes_trend(conn: sqlite3.Connection) -> None:
    ...
    snap = sql.get_readiness(conn, as_of=as_of.isoformat(), window_days=14)
...
def test_get_readiness_empty_window(conn: sqlite3.Connection) -> None:
    snap = sql.get_readiness(conn, as_of="2026-04-15", window_days=7)
...
def test_get_adherence_counts_and_totals(conn: sqlite3.Connection) -> None:
    ...
    rep = sql.get_adherence(conn, week_id="2026-W17")
...
def test_compare_plan_to_actual_missing_session_surfaces(conn: sqlite3.Connection) -> None:
    ...
    deltas = sql.compare_plan_to_actual(conn, week_id="2026-W17")
    assert len(deltas) == 1
    d = deltas[0]
