...
Pure functions: take a ``sqlite3.Connection`` plus filter args, return Pydantic
models. MCP tool wrappers in ``server.py`` open the connection via
``tempo.db.connect``. Separating the two makes tests trivial (seed a fixture
DB, call the pure function) and keeps tool registration declarative.
"""
...
def get_load_curve(
    ...
    *,
    ...
    ``sport`` filter is informational — all per-sport CTL columns are returned
    for every point, the agent picks which one to read.
    """
    rows = conn.execute(
    ...
    _ = sport  # retained for tool schema; no column to filter by.
    return [LoadPoint(**dict(r)) for r in rows]
...
def get_readiness(
    ...
    *,
    ...
    """Aggregate wellness_daily over the ``window_days`` ending at ``as_of``.

    Returns latest values plus 7d trend (7d mean vs previous 7d mean for HRV).
    """
    ...
    snap = ReadinessSnapshot(as_of=as_of, samples=len(rows))
    if not rows:
        return snap
...
def compare_plan_to_actual(conn: sqlite3.Connection, *, week_id: str) -> list[Delta]:
    rows = conn.execute(
        """
        SELECT sp.id AS planned_session_id,
    ...
    for r in rows:
        ...
        tss_delta = (
            None if planned_tss is None or actual_tss is None else actual_tss - planned_tss
        )
        planned_dur = r["target_duration_s"]
        actual_dur = r["actual_duration_s"]
        dur_delta = (
            None if planned_dur is None or actual_dur is None else actual_dur - planned_dur
        )
        deltas.append(
            Delta(
                planned_session_id=r["planned_session_id"],
...
def _now_iso() -> str:  # pragma: no cover - utility
